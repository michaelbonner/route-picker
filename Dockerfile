...
# =====================================================================
# Production image for WhichRouteIsFaster.com.
#
...
# production box with railpack, which pinned the CPU for the length of every
...
# `prepare` (`svelte-kit sync`), which the Vite plugin re-runs during the build.
...
RUN DATABASE_URL=postgres://build:build@127.0.0.1:5432/build \
    AUTH_SECRET=build_time_placeholder_not_used_at_runtime \
...
# the drizzle-kit migration nor the server runs with root inside the container.
...
# Runtime config (DATABASE_URL, AUTH_SECRET, BETTER_AUTH_URL, GITHUB_*,
# GOOGLE_*, ...) is injected by Dokploy as container env vars.
#
# `start` = `bun run db:migrate && bun run build/index.js`, exactly as before. A
# failed migration exits non-zero before the server binds, so the new container
# never becomes healthy and Dokploy keeps the previous one serving.
CMD ["bun", "run", "start"]
